# GeoLeadScraper — Yandex Maps AUTO + BATCH (v1.8.0)
...
3. **EXPORT RAW** сразу после завершения. Файл называется `RAW_ALL`, если сбор
   ...
   подозрительно рано.
4. **FILTER RAW → FINAL**.
5. **ДОЗАГРУЗИТЬ НУЖНЫЕ (N)** — телефоны, сайты и часы работы. Берутся только
   организации, которые прошли фильтр, плюс те, по которым фильтру не хватило
   данных для решения. Это единственный шаг, который запрашивает карточки, и его
   можно прервать и продолжить.
6. **ПЕРЕФИЛЬТРОВАТЬ RAW → FINAL** — чтобы организации, по которым не хватало
   данных, попали в реестр.
7. **EXPORT FINAL**.

Порядок важен: фильтр стоит копейки, а дозагрузка — запрос на организацию. На
реальной выдаче из 121 ресторана это 20 запросов вместо 121.
