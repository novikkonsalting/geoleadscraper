;(() => {
  ...
  const GROUP_TOKENS={
    ...
    // Food production is a third kind of organisation, not a shop and not a
    // place to eat. It only enters a register whose query list names the group,
    // so an existing общепит + розница register is untouched.
    'пищевое производство':['пищевое производство','производство продуктов питания','пищевая промышленность','хлебозавод','хлебокомбинат','кондитерская фабрика','производство кондитер','мясокомбинат','мясоперерабат','производство мяс','молочный завод','молочный комбинат','производство молоч','рыбоперерабат','рыбный комбинат','производство рыб','пивоварн','производство напитков','производство пива','производство полуфабрикат','полуфабрикаты собственного производства','пищевые ингредиенты','обжарка кофе','обжарочн','консервный завод','производство мороженого','производство снеков','производство круп','мельниц'],
    // Churches, mosques, synagogues and the like: a register of its own, and,
    // like production, it enters the scope only when a query list names it.
    'религиозные учреждения':['православн','храм','церков','часовн','монастыр','подворь','лавра','мечет','мусульман','исламск','медресе','синагог','иудей','католич','костел','протестант','лютеран','баптист','евангел','пятидесятн','адвентист','буддий','дацан','религиозн','духовн','молитвен','приход','епархи','патриарх','воскресная школа'],
  };
  const GROUP_LABELS={'общепит':'Общепит','продуктовая розница':'Продуктовая розница','пищевое производство':'Пищевое производство','религиозные учреждения':'Религиозные учреждения'};
  ...
  const categoryMatch = (category,categoriesRaw) => {
    ...
    switch(requested){
      ...
      case 'хлебозавод': return includesAny(actual,['хлебозавод','хлебокомбинат','производство хлеб']);
      case 'кондитерское производство': return includesAny(actual,['кондитерская фабрика','производство кондитер']);
      case 'мясопереработка': return includesAny(actual,['мясокомбинат','мясоперерабат','производство мяс']);
      case 'молочное производство': return includesAny(actual,['молочный завод','молочный комбинат','производство молоч']);
      case 'рыбопереработка': return includesAny(actual,['рыбоперерабат','рыбный комбинат','производство рыб']);
      case 'производство напитков': return includesAny(actual,['производство напитков','производство пива','пивоварн']);
      case 'пивоварня': return includesAny(actual,['пивоварн','производство пива']);
      case 'производство полуфабрикатов': return includesAny(actual,['полуфабрикат']);
      case 'пищевые ингредиенты / специи': return includesAny(actual,['пищевые ингредиенты','специи','пищевые добавк']);
      case 'обжарка кофе': return includesAny(actual,['обжарка кофе','обжарочн']);
      case 'производство продуктов питания':
      case 'пищевое производство': return includesAny(actual,['производство продуктов питания','пищевое производство','пищевая промышленность','хлебозавод','хлебокомбинат','кондитерская фабрика','мясокомбинат','мясоперерабат','молочный завод','молочный комбинат','рыбоперерабат','пивовар','производство напитков','производство хлеб','производство кондитер','производство мяс','производство молоч','производство рыб','производство полуфабрикат','пищевые ингредиенты','обжарка кофе','консервный завод','производство мороженого']);
      ...
      case 'православный храм': return includesAny(actual,['православн','храм','церков','приход','подворь','собор']);
      case 'часовня': return includesAny(actual,['часовн']);
      case 'монастырь': return includesAny(actual,['монастыр','подворь','лавра','скит']);
      case 'мечеть': return includesAny(actual,['мечет','мусульман','исламск','медресе']);
      case 'синагога': return includesAny(actual,['синагог','иудей']);
      case 'католический / протестантский храм': return includesAny(actual,['католич','костел','протестант','лютеран','баптист','евангел','пятидесятн','адвентист']);
      case 'буддийский храм': return includesAny(actual,['буддий','дацан','дуган']);
      case 'духовное учебное заведение': return includesAny(actual,['духовная семинари','духовное училищ','духовная академи','медресе','воскресная школа']);
      case 'религиозная организация':
      case 'религиозное / духовное учреждение': return includesAny(actual,['религиозн','храм','церков','мечет','синагог','монастыр','часовн','духовн','приход','подворь','молитвен','буддий','исламск','православ','католич','епархи','патриарх']);
      default: return null;
    }
  };
  const GEO_VERSION=6;
  ...
  const evaluateItem = async (item,forcedQuery=null) => {
    ...
    const detectedGroups=groups.map(g=>GROUP_LABELS[g]||g);
